...
const COMPANY_EMAIL = "danny.stephb@gmail.com";
...
function newLine(){return{id:"l"+Date.now()+Math.random().toString(36).slice(2),desc:"",qty:"1",price:""};}
...
function buildMondayEmailHTML(jobs) {
  const rows = jobs.map(j => `
    <tr>
      <td style="padding:8px 12px;border-bottom:1px solid #eee;font-weight:600">${j.job_ref}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #eee">${j.customer_name||""}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #eee">${j.po_number||"—"}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #eee">${j.description||""}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #eee;color:${isOverdue(j)?"#b83030":"#333"}">${fmt(j.due_date)}</td>
      <td style="padding:8px 12px;border-bottom:1px solid #eee;font-weight:600">${fmtGBP(lineTotal(j.lines))}</td>
    </tr>`).join("");
  return `
    <div style="font-family:Arial,sans-serif;max-width:700px">
      <div style="background:#0f2a4a;padding:20px 24px;border-bottom:3px solid #c9a84c">
        <h2 style="color:#fff;margin:0;font-size:18px">${COMPANY}</h2>
        <p style="color:#c8d0dc;margin:4px 0 0;font-size:13px">Monday Morning Production Report — ${fmt(todayStr())}</p>
      </div>
      <div style="padding:20px 24px">
        <p style="color:#3a4f66;font-size:14px">Good morning Danny, here are all jobs currently In Production:</p>
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead>
            <tr style="background:#0f2a4a">
              <th style="padding:8px 12px;text-align:left;color:#c8d0dc;font-size:11px;text-transform:uppercase">Job Ref</th>
              <th style="padding:8px 12px;text-align:left;color:#c8d0dc;font-size:11px;text-transform:uppercase">Customer</th>
              <th style="padding:8px 12px;text-align:left;color:#c8d0dc;font-size:11px;text-transform:uppercase">PO Number</th>
              <th style="padding:8px 12px;text-align:left;color:#c8d0dc;font-size:11px;text-transform:uppercase">Description</th>
              <th style="padding:8px 12px;text-align:left;color:#c8d0dc;font-size:11px;text-transform:uppercase">Due</th>
              <th style="padding:8px 12px;text-align:left;color:#c8d0dc;font-size:11px;text-transform:uppercase">Value</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
        <p style="color:#6a7f99;font-size:12px;margin-top:20px">${jobs.length} job${jobs.length!==1?"s":""} in production · Total value: ${fmtGBP(jobs.reduce((a,j)=>a+lineTotal(j.lines),0))}</p>
      </div>
      <div style="background:#f4f6f9;padding:12px 24px;border-top:1px solid #d0d8e4">
        <p style="color:#6a7f99;font-size:11px;margin:0">${COMPANY} · Job Management System ${VERSION}</p>
      </div>
    </div>`;
}
...
function useMondayReport(jobs) {
  useEffect(() => {
    const check = async () => {
      ...
      const lastSent = localStorage.getItem("rsm_monday_sent");
      const todayKey = todayStr();
      if (isMonday && isNineAM && lastSent !== todayKey) {
        const inProd = jobs.filter(j => j.status === "In Production");
        if (inProd.length > 0) {
          await sendEmail(NOTIFY_EMAIL, `Monday Production Report — ${fmt(todayKey)} — ${inProd.length} jobs in production`, buildMondayEmailHTML(inProd));
          localStorage.setItem("rsm_monday_sent", todayKey);
          // Also trigger print
          window.__mondayPrintJobs = inProd;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const jobSub = supabase.channel("jobs-channel")
      .on("postgres_changes", { event: "*", schema: "public", table: "jobs" }, () => loadJobs())
      .subscribe();
    const custSub = supabase.channel("custs-channel")
      .on("postgres_changes", { event: "*", schema: "public", table: "customers" }, () => loadCustomers())
      .subscribe();
    // Monday print listener
    const onMonday = () => { if(window.__mondayPrintJobs) setMondayPrint(window.__mondayPrintJobs); };
    window.addEventListener("mondayprint", onMonday);
    return () => { jobSub.unsubscribe(); custSub.unsubscribe(); window.removeEventListener("mondayprint", onMonday); };
  }, []);
  ...
  async function saveJob(j) {
    ...
    const payload = {
      job_ref: ref, customer_id: j.customer_id||null, customer_name: j.customer_name,
      contact_id: j.contact_id, contact_name: j.contact_name, description: j.description,
      po_number: j.po_number, status: j.status, priority: j.priority,
      quote_ref: j.quote_ref, quote_status: j.quote_status, lines: j.lines||[],
      stages: j.stages||[], stages_complete: j.stages_complete||{},
      drawing_number: j.drawing_number, drawing_attached: j.drawing_attached||false,
      date_received: j.date_received||todayStr(), due_date: j.due_date||null,
      invoice_ref: j.invoice_ref, notes: j.notes
    };
    if (isNew) {
      const { error } = await supabase.from("jobs").insert([payload]);
      if (error) { toast_("Error saving job: "+error.message, "warn"); return; }
  ...
  async function toggleStage(job, stage) {
    ...
    setJobs(p => p.map(j => j.id===job.id ? u : j));
    setJobModal(u);
  }
  ...
  async function removeStage(job, stage) {
    ...
    const u = { ...job, stages, stages_complete: sc };
    setJobs(p => p.map(j => j.id===job.id ? u : j));
    setJobModal(u);
  }
  ...
  function exportJobCSV(job) {
    ...
    a.download = `${job.job_ref}_${todayStr()}.csv`; a.click();
  }
  ...
  // ── MANUAL MONDAY REPORT ──────────────────────────────────────────────────
  async function sendMondayReport() {
    const inProd = jobs.filter(j => j.status === "In Production");
    if (inProd.length === 0) { toast_("No jobs currently in production", "warn"); return; }
    await sendEmail(NOTIFY_EMAIL, `Production Report — ${fmt(todayStr())} — ${inProd.length} jobs`, buildMondayEmailHTML(inProd));
  ...
  return (
    <div style={{minHeight:"100vh",background:C.silverPale,fontFamily:"'Trebuchet MS','Segoe UI',sans-serif",color:C.text}}>
      ...
      {dueTomorrow.length>0 && (
        <div style={{background:C.warnBg,borderBottom:`2px solid ${C.gold}`,padding:"8px 24px",display:"flex",alignItems:"center",gap:12}}>
          <span style={{fontWeight:700,fontSize:13,color:C.warn}}>⏰ Reminder:</span>
          <span style={{fontSize:13,color:C.warn}}>{dueTomorrow.length} job{dueTomorrow.length>1?"s":""} due tomorrow not yet ready: {dueTomorrow.map(j=>j.job_ref).join(", ")}</span>
        </div>
      )}
      ...
      {confirmDel && (
        <Overlay onClose={()=>setConfirmDel(null)}>
          <div style={{maxWidth:360}}>
            ...
            <div style={{fontSize:14,color:C.textMid,lineHeight:1.6}}>Permanently delete <strong>{confirmDel.job_ref}</strong>? This cannot be undone.</div>
      ...
      {laserPrompt && (
        <Overlay onClose={()=>{setEditJob(laserPrompt.job);setLaserPrompt(null);}}>
          <div style={{maxWidth:380,textAlign:"center"}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center"}}>
              <Btn primary onClick={()=>{
                const j={...laserPrompt.job,stages:["Laser Cutting",...(laserPrompt.job.stages||[]).filter(s=>s!=="Laser Cutting")]};
                saveJob(j); setLaserPrompt(null);
              }}>Yes — Add Laser Cutting</Btn>
      ...
      <main style={{padding:"24px 24px 48px"}}>
        <div style={{maxWidth:1280,margin:"0 auto"}}>
          ...
          {loading && <div style={{textAlign:"center",padding:"60px 0",color:C.textLight,fontSize:14}}>Loading data from database…</div>}
            ...
            <>
              ...
              {view==="dashboard" && (
                <>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:24}}>
                    {STATUS_FLOW.map(s=>(
                      <div key={s} style={{background:C.white,border:`1px solid ${C.border}`,borderTop:`3px solid ${SC[s].border}`,borderRadius:8,padding:"16px 20px",cursor:"pointer"}} onClick={()=>{setView("jobs");setFilterStatus(s);}}>
                        <div style={{fontSize:30,fontWeight:800,color:SC[s].color,lineHeight:1}}>{counts[s]}</div>
                        <div style={{fontSize:11,color:C.textLight,marginTop:4,fontWeight:600,letterSpacing:0.3}}>{s}</div>
                      </div>
                    ))}
                    <div style={{background:C.white,border:`1px solid ${C.border}`,borderTop:`3px solid ${C.gold}`,borderRadius:8,padding:"16px 20px"}}>
                      <div style={{fontSize:24,fontWeight:800,color:C.gold,lineHeight:1}}>{fmtGBP(pipeline)}</div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16,marginBottom:24}}>
                    <ACard title="Overdue Jobs" count={overdue.length} color={C.danger}>
                      {overdue.length===0?<Em>All jobs on schedule ✓</Em>:overdue.map(j=>(
                        <ARow key={j.id} job={j} onClick={()=>setJobModal(j)}><span style={{color:C.danger,fontSize:12}}>Due {fmt(j.due_date)}</span></ARow>
                      ))}
                    </ACard>
                    <ACard title="Ready to Despatch — Awaiting Invoice" count={uninvoiced.length} color={C.accent}>
                      {uninvoiced.length===0?<Em>No outstanding invoices ✓</Em>:uninvoiced.map(j=>(
                        <ARow key={j.id} job={j} onClick={()=>setJobModal(j)}><span style={{color:C.accent,fontSize:12}}>{fmtGBP(lineTotal(j.lines))}</span></ARow>
                      ))}
                    </ACard>
                    {dueTomorrow.length>0&&(
                      <ACard title="Due Tomorrow — Not Ready" count={dueTomorrow.length} color={C.warn}>
                        {dueTomorrow.map(j=><ARow key={j.id} job={j} onClick={()=>setJobModal(j)}><span style={{color:C.warn,fontSize:12}}>{j.status}</span></ARow>)}
                      </ACard>
                    )}
                  </div>
                ...
                <>
                  ...
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
                    {["All",...STATUS_FLOW].map(s=>(
                      <button key={s} style={{background:filterStatus===s?C.navy:C.white,color:filterStatus===s?C.white:C.textMid,border:`1px solid ${filterStatus===s?C.navy:C.border}`,borderRadius:20,padding:"4px 14px",fontFamily:"inherit",fontSize:12,fontWeight:600,cursor:"pointer"}} onClick={()=>setFilterStatus(s)}>{s}</button>
                    ))}
                  </div>
                      ...
                      return(
                        <div key={qs} style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:8,overflow:"hidden"}}>
                          <div style={{padding:"10px 16px",background:hc,color:tc,fontWeight:700,fontSize:13,display:"flex",justifyContent:"space-between"}}>
                            <span>{qs}</span><span style={{fontWeight:400,fontSize:12}}>{qj.length} · {fmtGBP(tot)}</span>
                          </div>
                          ...
                          {qj.map(j=>(
                            <div key={j.id} style={{padding:"12px 16px",borderTop:`1px solid ${C.borderLight}`,cursor:"pointer"}} onClick={()=>setJobModal(j)}>
                              <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                                <Ref>{j.job_ref}</Ref>
                                <span style={{fontWeight:700,color:C.navy,fontSize:13}}>{fmtGBP(lineTotal(j.lines))}</span>
                              </div>
                        ...
                        <div key={c.id} style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:8,padding:16,display:"flex",gap:14,alignItems:"flex-start"}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{display:"flex",flexWrap:"wrap",gap:12,marginBottom:8}}>
                              {(c.contacts||[]).map(ct=>(
                                <div key={ct.id} style={{fontSize:12,color:C.textMid}}>👤 {ct.name}{ct.phone?` · ${ct.phone}`:""}{ct.email?` · ${ct.email}`:""}</div>
                              ))}
                            </div>
              ...
              {view==="pricing" && (
                <>
                  ...
                  {!priceSearch&&<Em center>Type a keyword above to search pricing history across all jobs.</Em>}
                </>
              )}
              ...
              {view==="alerts" && (
                <>
                  ...
                  {dueTomorrow.length>0&&<><ACard title="Due Tomorrow — Not Yet Ready to Despatch" count={dueTomorrow.length} color={C.warn}>{dueTomorrow.map(j=><ALarge key={j.id} job={j} onClick={()=>setJobModal(j)}/>)}</ACard><div style={{height:16}}/></>}
                  ...
                  <div style={{height:16}}/>
                  <ACard title="Ready to Despatch — Awaiting Invoice" count={uninvoiced.length} color={C.accent}>{uninvoiced.length===0?<Em>All ready jobs invoiced ✓</Em>:uninvoiced.map(j=><ALarge key={j.id} job={j} onClick={()=>setJobModal(j)} extra={fmtGBP(lineTotal(j.lines))}/>)}</ACard>
                </>
              )}
            </>
          )}
      ...
      {/* CUSTOMER HISTORY */}
      {custHistory&&(
        <Overlay onClose={()=>setCustHistory(null)}>
          <div style={{minWidth:300,maxWidth:700}}>
            ...
            {jobs.filter(j=>j.customer_id===custHistory.id).length===0
              ?<Em>No jobs found for this customer.</Em>
              :jobs.filter(j=>j.customer_id===custHistory.id).map(j=>(
                <div key={j.id} style={{border:`1px solid ${C.border}`,borderRadius:8,marginBottom:12,overflow:"hidden"}}>
                  ...
                  <div style={{padding:"8px 14px"}}>
                    {(j.lines||[]).map(l=>(
                      <div key={l.id} style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"4px 0",borderBottom:`1px solid ${C.borderLight}`}}>
      ...
      {editJob&&(
        <Overlay onClose={()=>setEditJob(null)}>
          <JobForm job={editJob} customers={customers}
            onSave={(j)=>{
...
// ── JOB DETAIL ────────────────────────────────────────────────────────────────
function JobDetail({job,onEdit,onAdvance,onDelete,onToggleStage,onAddStage,onRemoveStage,onPrint,onDelivery,stageInput,setStageInput}){
  ...
  return(
    <div style={{minWidth:300,maxWidth:620}}>
      ...
      <div style={{background:C.silverPale,border:`1px solid ${C.border}`,borderRadius:8,padding:14,marginBottom:14}}>
        ...
        {(job.lines||[]).map(l=>(
          <div key={l.id} style={{display:"flex",gap:10,padding:"5px 0",borderBottom:`1px solid ${C.borderLight}`,fontSize:13}}>
            <span style={{flex:1}}>{l.desc||"—"}</span>
            <span style={{color:C.textLight,whiteSpace:"nowrap"}}>x{l.qty}</span>
            <span style={{color:C.textLight,whiteSpace:"nowrap",minWidth:60,textAlign:"right"}}>{fmtGBP(l.price)} ea</span>
      ...
      <div style={{background:C.silverPale,border:`1px solid ${C.border}`,borderRadius:8,padding:14,marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",fontWeight:700,fontSize:13,color:C.navy,marginBottom:8}}>
          <span>Production Stages</span>{pct!==null&&<span style={{fontSize:12,color:C.textLight}}>{done}/{allS.length}</span>}
        </div>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
          {PRESET_STAGES.filter(s=>!allS.includes(s)).map(s=>(
            <button key={s} style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:20,padding:"2px 10px",fontSize:11,fontFamily:"inherit",cursor:"pointer",color:C.textMid}} onClick={()=>onAddStage(job,s)}>+ {s}</button>
          ))}
        </div>
  ...
  return(
    <div style={{minWidth:300,maxWidth:640}}>
      ...
      <div style={{background:C.silverPale,border:`1px solid ${C.border}`,borderRadius:8,padding:14,marginBottom:16}}>
        ...
        {f.lines.map(l=>(
          <div key={l.id} style={{display:"grid",gridTemplateColumns:"1fr 60px 80px 24px",gap:6,marginBottom:6,alignItems:"center"}}>
            <input style={S.inp} value={l.desc} onChange={e=>setLine(l.id,"desc",e.target.value)} placeholder="Item description"/>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:8}}>
          <button style={{background:"none",border:`1px solid ${C.border}`,borderRadius:5,padding:"5px 12px",fontFamily:"inherit",fontSize:12,cursor:"pointer",color:C.accent}} onClick={addLine}>+ Add Line</button>
  ...
  return(
    <div style={{background:"#fff",minHeight:"100vh",padding:32,fontFamily:"Arial,sans-serif",color:"#000"}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        ...
        <tbody>
          {jobs.map(j=>(
            <tr key={j.id} style={{borderBottom:"1px solid #eee",...(isOverdue(j)?{background:"#fff5f5"}:{})}}>
              <td style={{padding:"8px 12px",fontWeight:600}}>{j.job_ref}</td>
              <td style={{padding:"8px 12px"}}>{j.customer_name}</td>
              <td style={{padding:"8px 12px",color:"#666"}}>{j.po_number||"—"}</td>
              <td style={{padding:"8px 12px"}}>{j.description}</td>
              <td style={{padding:"8px 12px",color:isOverdue(j)?"#b83030":"#333",fontWeight:isOverdue(j)?700:400}}>{fmt(j.due_date)}</td>
              <td style={{padding:"8px 12px",fontWeight:600}}>{fmtGBP(lineTotal(j.lines))}</td>
              <td style={{padding:"8px 12px"}}>{j.priority}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div style={{marginTop:16,fontWeight:700,textAlign:"right",fontSize:13}}>Total In Production: {fmtGBP(jobs.reduce((a,j)=>a+lineTotal(j.lines),0))}</div>
      <div style={{marginTop:40,fontSize:10,color:"#bbb",borderTop:"1px solid #eee",paddingTop:10}}>{COMPANY} · Production Report · {fmt(todayStr())} · {VERSION}</div>
    </div>
  );
}
  ...
  return(
    <div style={{background:"#fff",minHeight:"100vh",padding:32,fontFamily:"Arial,sans-serif",color:"#000"}}>
      ...
      <div style={{borderBottom:"3px solid #0f2a4a",paddingBottom:16,marginBottom:20,display:"flex",justifyContent:"space-between"}}>
        <div><div style={{fontSize:20,fontWeight:700,color:"#0f2a4a"}}>{COMPANY}</div><div style={{fontSize:11,color:"#666",marginTop:2}}>{COMPANY_ADDR} · {COMPANY_TEL}</div><div style={{fontSize:11,color:"#666"}}>Job Sheet</div></div>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",marginBottom:20,fontSize:12}}>
        <tbody>
          {[["Customer",job.customer_name],["Contact",job.contact_name],["PO Number",job.po_number],["Due Date",fmt(job.due_date)],["Drawing No",job.drawing_number],["Drawing Attached",job.drawing_attached?"Yes":"No"],["Status",job.status],["Notes",job.notes]].filter(([,v])=>v).map(([l,v])=>(
      ...
      <table style={{width:"100%",borderCollapse:"collapse",marginBottom:20,fontSize:12}}>
        ...
        <tbody>
          {(job.lines||[]).map(l=><tr key={l.id} style={{borderBottom:"1px solid #eee"}}><td style={{padding:"6px 10px"}}>{l.desc}</td><td style={{padding:"6px 10px",textAlign:"center"}}>{l.qty}</td><td style={{padding:"6px 10px",textAlign:"right"}}>{fmtGBP(l.price)}</td><td style={{padding:"6px 10px",textAlign:"right",fontWeight:600}}>{fmtGBP(parseFloat(l.qty||1)*parseFloat(l.price||0))}</td></tr>)}
      ...
      <div style={{marginTop:36,fontSize:11,color:"#999",borderTop:"1px solid #ddd",paddingTop:10}}>{COMPANY} · Job Sheet {job.job_ref} · {VERSION}</div>
    </div>
  );
}
  ...
  return(
    <div style={{background:"#fff",minHeight:"100vh",padding:32,fontFamily:"Arial,sans-serif",color:"#000"}}>
      ...
      <div style={{borderBottom:"3px solid #c9a84c",paddingBottom:16,marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        ...
        <div style={{textAlign:"right"}}><div style={{fontSize:11,color:"#888",letterSpacing:1,textTransform:"uppercase"}}>Delivery Note</div><div style={{fontSize:22,fontWeight:700,color:"#0f2a4a"}}>{job.job_ref}</div><div style={{fontSize:11,color:"#666"}}>Date: {fmt(todayStr())}</div></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:24}}>
        ...
        <div><table style={{width:"100%",fontSize:12,borderCollapse:"collapse"}}><tbody>{[["Job Ref",job.job_ref],["PO Number",job.po_number||"—"],["Drawing No",job.drawing_number||"—"],["Drawing Attached",job.drawing_attached?"Yes":"No"]].map(([l,v])=><tr key={l}><td style={{padding:"3px 8px",background:"#f5f5f5",fontWeight:600,width:110}}>{l}</td><td style={{padding:"3px 8px",borderBottom:"1px solid #eee"}}>{v}</td></tr>)}</tbody></table></div>
      </div>
      ...
      <div style={{marginTop:40,fontSize:10,color:"#bbb",borderTop:"1px solid #eee",paddingTop:10,textAlign:"center"}}>{COMPANY} · {COMPANY_ADDR} · Delivery Note {job.job_ref} · {VERSION}</div>
    </div>
  );
}
...
function Btn({children,onClick,primary,ghost,danger,small}){const base={border:"none",borderRadius:5,fontFamily:"inherit",fontWeight:600,cursor:"pointer",padding:small?"5px 12px":"8px 16px",fontSize:small?12:13,letterSpacing:0.2};const v=primary?{background:C.navy,color:"#fff"}:danger?{background:C.dangerBg,color:C.danger,border:`1px solid ${C.danger}40`}:ghost?{background:"transparent",color:C.textMid,border:`1px solid ${C.border}`}:{background:C.silverLight,color:C.navy,border:`1px solid ${C.border}`};return<button style={{...base,...v}} onClick={onClick}>{children}</button>;}
function Overlay({children,onClose}){return<div style={{position:"fixed",inset:0,background:"#00000066",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}} onClick={onClose}><div style={{background:C.white,border:`1px solid ${C.border}`,borderTop:`4px solid ${C.navy}`,borderRadius:10,padding:28,maxWidth:680,width:"100%",position:"relative",boxShadow:"0 8px 40px #0003",maxHeight:"90vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}><button style={{position:"absolute",top:12,right:14,background:"none",border:"none",color:C.textLight,fontSize:17,cursor:"pointer"}} onClick={onClose}>✕</button>{children}</div></div>;}
